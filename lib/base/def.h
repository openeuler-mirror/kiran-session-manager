...
{
public:
    KSMDefer(std::function<void(std::string)> func, std::string fun_name) : func_(func),
                                                                            fun_name_(fun_name) {}
    ~KSMDefer() { func_(fun_name_); }
...
// helper macro for KSMDefer class
#define SCOPE_EXIT(block) KSMDefer CONNECT(_defer_, __LINE__)([&](std::string _arg_function) block, __FUNCTION__)
