...
KSMPresence::KSMPresence() : enabled_idle_timeout_(true),
                             idle_timeout_(0),
...
void KSMPresence::init()
{
    this->idle_timeout_ = this->settings_->get_int(KSM_SCHEMA_KEY_IDLE_DELAY);
    this->idle_xlarm_.init();
    this->update_idle_xlarm();

    this->idle_xlarm_.signal_alarm_triggered().connect(sigc::mem_fun(this, &KSMPresence::on_alarm_triggered_cb));
    this->idle_xlarm_.signal_alarm_reset().connect(sigc::mem_fun(this, &KSMPresence::on_alarm_reset_cb));
    this->settings_->signal_changed().connect(sigc::mem_fun(this, &KSMPresence::on_settings_changed));

    this->dbus_connect_id_ = Gio::DBus::own_name(Gio::DBus::BUS_TYPE_SESSION,
                                                 KSM_DBUS_NAME,
                                                 sigc::mem_fun(this, &KSMPresence::on_bus_acquired),
                                                 sigc::mem_fun(this, &KSMPresence::on_name_acquired),
                                                 sigc::mem_fun(this, &KSMPresence::on_name_lost));
}

void KSMPresence::enable_idle_timeout(bool enabled)
{
...
void KSMPresence::SetStatus(guint32 status, MethodInvocation &invocation)
{
...
void KSMPresence::SetStatusText(const Glib::ustring &status_text, MethodInvocation &invocation)
{
...
bool KSMPresence::status_setHandler(guint32 value)
{
...
bool KSMPresence::status_text_setHandler(const Glib::ustring &value)
{
...
void KSMPresence::update_idle_xlarm()
{
...
void KSMPresence::on_alarm_triggered_cb(std::shared_ptr<XAlarmInfo> xalarm)
{
    KLOG_PROFILE("");
...
void KSMPresence::on_alarm_reset_cb()
{
    KLOG_PROFILE("");
    this->status_set(KSMPresenceStatus::KSM_PRESENCE_STATUS_AVAILABLE);
}

void KSMPresence::on_settings_changed(const Glib::ustring &key)
{
    KLOG_PROFILE("");
...
void KSMPresence::on_bus_acquired(const Glib::RefPtr<Gio::DBus::Connection> &connect, Glib::ustring name)
{
    if (!connect)
    {
...
void KSMPresence::on_name_acquired(const Glib::RefPtr<Gio::DBus::Connection> &connect, Glib::ustring name)
{
    KLOG_DEBUG("Success to register dbus name: %s", name.c_str());
}

void KSMPresence::on_name_lost(const Glib::RefPtr<Gio::DBus::Connection> &connect, Glib::ustring name)
{
