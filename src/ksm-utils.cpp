...
std::string KSMUtils::generate_startup_id()
{
    ...
    if (!rand1)
    {
        rand1 = KSMUtils::rand_.get_int();
        rand2 = KSMUtils::rand_.get_int();
        pid = getpid();
    }
...
std::vector<std::string> KSMUtils::get_autostart_dirs()
{
...
void KSMUtils::setenv(const std::string &key, const std::string &value)
{
    KLOG_DEBUG("key: %s, value: %s.", key.c_str(), value.c_str());

    Glib::setenv(key, value);
    KSMUtils::setenv_to_dbus(key, value);
    KSMUtils::setenv_to_systemd(key, value);
}

void KSMUtils::setenvs(const std::map<Glib::ustring, Glib::ustring> &envs)
{
    try
    {
...
uint32_t KSMUtils::generate_cookie()
{
    return KSMUtils::rand_.get_int_range(1, G_MAXINT32);
}

KSMPhase KSMUtils::phase_str2enum(const std::string &phase_str)
{
...
std::string KSMUtils::phase_enum2str(KSMPhase phase)
{
    switch (phase)
    {
...
void KSMUtils::setenv_to_dbus(const std::string &key, const std::string &value)
{
    try
    {
        auto daemon_proxy = Gio::DBus::Proxy::create_for_bus_sync(Gio::DBus::BUS_TYPE_SESSION,
...
void KSMUtils::setenv_to_systemd(const std::string &key, const std::string &value)
{
    try
    {
        auto systemd_proxy = Gio::DBus::Proxy::create_for_bus_sync(Gio::DBus::BUS_TYPE_SESSION,
