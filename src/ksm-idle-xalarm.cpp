...
KSMIdleXAlarm::KSMIdleXAlarm() : xdisplay_(NULL),
                                 sync_event_base_(0),
                                 sync_error_base_(0),
                                 idle_counter_(None),
...
KSMIdleXAlarm::~KSMIdleXAlarm()
{
    if (this->added_event_filter_)
    {
        gdk_window_remove_filter(NULL, &KSMIdleXAlarm::on_event_filter_cb, this);
    }
...
void KSMIdleXAlarm::init()
{
    ...
    gdk_window_add_filter(NULL, &KSMIdleXAlarm::on_event_filter_cb, this);
...
bool KSMIdleXAlarm::set(XAlarmType type, uint32_t timeout)
{
...
bool KSMIdleXAlarm::unset(XAlarmType type)
{
...
int64_t KSMIdleXAlarm::get_xidle_time()
{
...
bool KSMIdleXAlarm::add(std::shared_ptr<XAlarmInfo> xalarm)
{
    RETURN_VAL_IF_FALSE(xalarm, false);
...
bool KSMIdleXAlarm::remove(std::shared_ptr<XAlarmInfo> xalarm)
{
    RETURN_VAL_IF_FALSE(xalarm, false);
...
void KSMIdleXAlarm::reset_all_xalarm()
{
    KLOG_PROFILE("");
...
void KSMIdleXAlarm::register_xalarm_by_xsync(std::shared_ptr<XAlarmInfo> xalarm, XSyncTestType test_type)
{
...
void KSMIdleXAlarm::unregister_xalarm_by_xsync(std::shared_ptr<XAlarmInfo> xalarm)
{
...
std::shared_ptr<XAlarmInfo> KSMIdleXAlarm::find_xalarm_by_type(XAlarmType type)
{
    KLOG_PROFILE("type: %d", type);
...
std::shared_ptr<XAlarmInfo> KSMIdleXAlarm::find_xalarm_by_id(XSyncAlarm xalarm_id)
{
...
int64_t KSMIdleXAlarm::xsyncvalue_to_int64(XSyncValue value)
{
    return ((int64_t)XSyncValueHigh32(value)) << 32 | (int64_t)XSyncValueLow32(value);
}

GdkFilterReturn KSMIdleXAlarm::on_event_filter_cb(GdkXEvent *gdkxevent, GdkEvent *event, gpointer data)
{
    XEvent *xevent = (XEvent *)gdkxevent;
    KSMIdleXAlarm *self = (KSMIdleXAlarm *)data;
    XSyncValue add;
    int overflow = 0;
