...
namespace Kiran
{
KSMAppManager::KSMAppManager()
{
    this->settings_ = Gio::Settings::create(KSM_SCHEMA_ID);
}

KSMAppManager *KSMAppManager::instance_ = nullptr;
void KSMAppManager::global_init()
{
    instance_ = new KSMAppManager();
    instance_->init();
}

std::shared_ptr<KSMApp> KSMAppManager::get_app_by_startup_id(const std::string &startup_id)
{
...
KSMAppVec KSMAppManager::start_apps(KSMPhase phase)
{
    KSMAppVec apps;
...
void KSMAppManager::init()
{
    this->load_apps();
}

void KSMAppManager::load_apps()
{
    this->load_autostart_apps();
    this->load_required_apps();
}

void KSMAppManager::load_required_apps()
{
    // 会话后端
...
void KSMAppManager::load_autostart_apps()
{
    for (auto autostart_dir : KSMUtils::get_autostart_dirs())
    {
        try
        {
...
bool KSMAppManager::add_app(Glib::RefPtr<Gio::DesktopAppInfo> app_info)
{
    RETURN_VAL_IF_FALSE(app_info, false);
    RETURN_VAL_IF_TRUE(app_info->gobj() == NULL, false);

    auto app = std::make_shared<KSMApp>(app_info);
...
KSMAppVec KSMAppManager::get_apps_by_phase(KSMPhase phase)
{
    KSMAppVec apps;
    for (auto iter : this->apps_)
    {
