...
namespace Kiran
{
KSMInhibitors::KSMInhibitors()
{
}

std::shared_ptr<KSMInhibitor> KSMInhibitors::add_inhibitor(const std::string &app_id,
                                                           ...
                                                           uint32_t flags)
{
    ...
    {
        auto cookie = KSMUtils::generate_cookie();
    ...
    auto inhibitor = std::make_shared<KSMInhibitor>(KSMInhibitor{.cookie = cookie,
                                                                 .app_id = app_id,
...
bool KSMInhibitors::add_inhibitor(std::shared_ptr<KSMInhibitor> inhibitor)
{
...
void KSMInhibitors::delete_inhibitor(uint32_t cookie)
{
    this->inhibitors_.erase(cookie);
    this->update_presence();
}

bool KSMInhibitors::has_inhibitor(uint32_t flags)
{
    auto iter = std::find_if(this->inhibitors_.begin(), this->inhibitors_.end(), [flags](std::pair<uint32_t, std::shared_ptr<KSMInhibitor>> iter) {
...
void KSMInhibitors::update_presence()
{
    auto presence = KSMManager::get_instance()->get_presence();
